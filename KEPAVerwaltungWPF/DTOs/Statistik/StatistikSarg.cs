namespace KEPAVerwaltungWPF.DTOs.Statistik;

public class StatistikSarg
{
    public string Spieler { get; set; } = string.Empty;
    public int AnzTeilnahmen { get; set; } = 0;
    public int Zehn { get; set; } = 0;
    public int Neun { get; set; } = 0;
    public int Acht { get; set; } = 0;
    public int Sieben { get; set; } = 0;
    public int Sechs { get; set; } = 0;
    public int Fünf { get; set; } = 0;
    public int Vier { get; set; } = 0;
    public int Drei { get; set; } = 0;
    public int Zwei { get; set; } = 0;
    public int Eins { get; set; } = 0;
}