...
public class MitgliedAktualisierenValidation : AbstractValidator<MitgliedDetails>
{
    public MitgliedAktualisierenValidation()
    {
        ...
        RuleFor(mitglied => mitglied.PassivSeit).GreaterThan(gt => gt.MitgliedSeit).WithMessage("PassivSeit muss nach MitgliedSeit liegen")
            .GreaterThan(gt => gt.Geburtsdatum).WithMessage("PassivSeit muss nach Geburtsdatum liegen");
        RuleFor(mitglied => mitglied.AusgeschiedenAm).GreaterThan(gt => gt.MitgliedSeit).WithMessage("AusgeschiedenAm muss nach MitgliedSeit liegen")
            .GreaterThan(gt => gt.Geburtsdatum).WithMessage("AusgeschiedenAm muss nach Geburtsdatum liegen");
    }
}